...
// List of public YouTube MP4 downloader APIs
const videoApis = [
  { label: "David CyrilTech", value: "https://api.davidcyriltech.my.id/download/ytmp4?url=" },
  { label: "Dark Yasiya", value: "https://www.dark-yasiya-api.site/download/ytmp4?url=" },
  { label: "GiftedTech", value: "https://api.giftedtech.web.id/api/download/dlmp4?url=" },
  { label: "Dreaded Site", value: "https://api.dreaded.site/api/ytdl/video?url=" },
];

// Bouncy animated button
...
function YouTubeVideoWidget() {
  const [selectedApi, setSelectedApi] = useState(videoApis[0].value);
  ...
  return (
    <form onSubmit={handleDownload}>
      <ChoiceSelector
        label="Choose Video Downloader API"
        options={videoApis}
        value={selectedApi}
      ...
      <DownloadButton type="submit" disabled={loading}>
        {loading ? "Generating..." : "Get MP4 Link"}
      </DownloadButton>
